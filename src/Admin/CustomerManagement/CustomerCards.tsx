...
const StatsCard = ({ card, maskId }: { card: (typeof statsCards)[0]; maskId: string }) => {
  ...
  return (
    <div 
      className="relative flex-1 min-w-[180px] m-2" 
    ...
    >
      ...
      <div className="absolute top-0 left-0 w-full h-full p-4 flex flex-col justify-between">
        <div>
          {/* Title */}
          <h3 className="text-sm font-medium text-[#171C35] mb-1">{card.title}</h3>
          {/* Subtitle */}
          <p className="text-xs text-[#171C35] opacity-70">{card.subtitle}</p>
        </div>

        {/* Value */}
        <div className="mb-2">
          <span className="text-3xl font-semibold text-[#171C35]">{card.value}</span>
        </div>
